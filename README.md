...
Daily aggregator of Nevada-located taildragger classified listings,
...
repos' approach, with the state filter swapped to "NV".
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, NV` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in Nevada"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/nevada/`
...
```html
<iframe
  src="https://taildraggers.github.io/nevada/"
  title="Taildragger Ads in Nevada"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_NEVADA_LOCATION_RE` in `main.py`, and updating
  `PAGE_TITLE`.
