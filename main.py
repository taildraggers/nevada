"""Aggregates Nevada-located listings from every taildraggers.com
...
ones whose location ends in ", NV".
...
Minnesota/Mississippi/Missouri/Montana/Nebraska repos' approach, with the
state filter swapped to "NV".
"""
...
PAGE_TITLE = "Taildragger Ads in Nevada"
STATE_ABBR = "NV"
...
_NEVADA_LOCATION_RE = re.compile(r",\s*NV\s*$", re.IGNORECASE)


def _is_nevada(location: str) -> bool:
    return bool(_NEVADA_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        nevada_rows = [row for row in rows if _is_nevada(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(nevada_rows)} in Nevada")
        all_listings.extend(nevada_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No Nevada listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique Nevada listings: {len(listings)}")
